...
apply. hukou selects the platform asset, verifies the publisher checksum
(fail-closed when the release has none unless you pass `--allow-unverified`;
see [Security model](#security-model)), and replaces the live file with an
atomic rename:
...
- **Supply-chain-aware installs.** When a release publishes a checksum asset,
  ...
  When the release has no checksum asset at all, upgrade **refuses by default**
  (fail-closed); operators may pass `--allow-unverified` for an explicit bypass,
  which prints and persists a durable `UNVERIFIED` audit marker
  (`checksum_verified: false` plus the computed asset SHA-256) rather than
  pretending anything was verified. Downloaded-asset and activated-binary
  hashes are tracked separately, and the installer verifies GitHub
  build-provenance attestation against an anchored signer identity. See
...
| `doctor receipt [name…]` | no | no | Per-tool DependencyReceipt view inside `doctor`: adopted version, live observation, checksum audit status, drift, rollback targets; `--json`, non-zero on drift unless `--no-fail-on-drift` |
| `upgrade [name…]` | yes | yes | Verify and replace; `--dry-run` previews, `--all` for every entry; no publisher checksum refuses unless `--allow-unverified` |
| `rollback <name>` | yes | no | Activate a retained version; `--to <tag>` or `--to original` |
| `up` | manifest/live binaries via each manager's own upgrade command, plus snapshot history under the data root; `--dry-run` writes nothing | via managers (their upgrade commands); `--dry-run` none | Upgrade every known manager and diff a before/after full-machine inventory snapshot; any manager failure or a snapshot-persistence failure exits 1 |
| `export` | no | no | Write a portable, machine-readable list of every adopted tool (JSON to stdout or `--output FILE`), for `import` on another machine |
| `suggest <name\|path>` | no | yes (search + latest-release only) | Suggest GitHub repositories for an unmanaged binary, ranked by exact-name match and stars, with ready-to-run `adopt` commands; advisory only, never applied automatically |
| `import <FILE>` | yes | no | Re-register tools from an exported list; the executables must already exist on PATH and the full adoption inspection runs again; `--dry-run` writes nothing |
...
- When a checksum asset exists, a missing, invalid, or mismatched entry for the
  ...
  a verification gap. A release with no checksum asset at all is also refused by
  default; only an explicit `--allow-unverified` continues, with loud
  `UNVERIFIED` markers on stdout/stderr and a durable `checksum_verified: false`
  (plus the computed asset SHA-256) in the manifest/transaction audit trail.
  `--allow-unverified` never bypasses a present-but-unusable checksum.
...
| `HUKOU_LANG` | Select the UI language (`zh` for Chinese); when unset, hukou follows the system locale, including the macOS GUI locale (a Chinese system shows a Chinese UI automatically) |
| `HUKOU_DOWNLOAD_MIN_SPEED` | Download throughput floor (bytes/s, default 32768); below it for the grace period a download counts as stalled |
| `HUKOU_DOWNLOAD_GRACE` | Stall grace period (seconds, default 5) before a slow download is retried through the system proxy |
