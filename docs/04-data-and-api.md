...
| `checksum_verified` | `true` only when checksum verification succeeded; otherwise omitted |
...
4. When there is no checksum asset, `asset_sha256` is still computed and
   saved, but `checksum_verified` must never be falsely set to true.
5. The manifest is only saved once both activation and audit fields are ready.
