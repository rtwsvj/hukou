...
| [`ADR-0006`](adr/ADR-0006-transaction-residue-self-heal.md) | Accepted | 未知非目录事务条目隔离自愈（未知目录保持 fail-closed）；新增 purge-quarantine 与身份绑定的 clean-live-temps repair 动作 |
...
- 2026-07-15：未知**非目录**事务条目不再楔死恢复，移入 `quarantined-<16hex>` 容器（META 记录原名）并保留证据；未知目录保持 fail-closed（可能是新版本 journal）。新增 `purge-quarantine` 与 `clean-live-temps`（逐项身份绑定 + 无活跃写入者门禁 + live 精确路径永久排除）两个 repair 动作。
