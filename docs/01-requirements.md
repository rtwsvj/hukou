...
3. **Fail closed**: must fail when a checksum file is found but no entry
   for the selected asset can be located, the entry is invalid, or the
   digest mismatches; and must also refuse by default when the release
   publishes **no** checksum asset at all. The only escape hatch is an
   explicit `--allow-unverified` on `upgrade`, which must print and
   persist a durable UNVERIFIED audit marker and must never bypass a
   present-but-unusable checksum.
