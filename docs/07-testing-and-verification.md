...
- A checksum mismatch.
- The release publishes no checksum asset: upgrade refuses by default, and
  only `--allow-unverified` installs while persisting `checksum_verified:
  false` plus a loud UNVERIFIED marker (`--allow-unverified` must not bypass
  mismatch or missing-entry cases).
