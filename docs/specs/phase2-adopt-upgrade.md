...
- Verification: when the release ships an `<asset>.sha256`/
  ...
  does not match, this must fail closed — and `--allow-unverified` must
  not bypass that path. When the release has **no** checksum asset at all,
  the default is also fail-closed (error points at `--allow-unverified`).
  Only with that explicit bypass may install continue: `asset_sha256` is
  still computed, stdout/stderr mark `UNVERIFIED`, and
  `checksum_verified=false` is persisted (never falsely set to true). Any
  verification failure aborts without touching the existing installation.
