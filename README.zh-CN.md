...
build-provenance attestation（用 `--signer-workflow` 钉在本仓库 release
workflow 上）；否则打印警告并回退到仅传输信任（HTTPS + 同源 checksums.txt）。
设置 `HUKOU_REQUIRE_ATTESTATION=1`（或 `true`/`yes`）可把该验证改为强制；其余
取值一律拒绝。当前证据和待办见
...
仍受账户 billing/spending 限制影响，因此这里不声称远端绿色。
