...
校验发布方 checksum（Release 无 checksum 时默认拒绝，除非传 `--allow-unverified`；
见[安全模型](#安全模型)），并用原子 rename 替换活跃文件：
...
- **供应链感知的安装。** Release 提供 checksum 资产时，所选资产的条目缺失、
  无效或不匹配一律失败关闭。Release 完全没有 checksum 资产时，upgrade **默认
  拒绝**（fail-closed）；可用 `--allow-unverified` 显式旁路，旁路时在输出与
  落盘审计中显著标注 `UNVERIFIED`（`checksum_verified: false` 加上算出的资产
  SHA-256），而不是假装校验过。下载资产与激活二进制的 hash 分开记录；安装器
  按锚定的签名者身份验证 GitHub build-provenance attestation。见
...
| `receipt [name…]` | 否 | 否 | 本地 DependencyReceipt：收编版本、现状观测、checksum 审计状态、漂移、回滚目标；`--json`；有漂移默认非零退出（可用 `--no-fail-on-drift`） |
| `upgrade [name…]` | 是 | 是 | 校验并替换；`--dry-run` 预览，`--all` 全部条目；无发布方 checksum 默认拒绝（`--allow-unverified` 旁路） |
| `rollback <name>` | 是 | 否 | 激活某个保留版本；`--to <tag>` 或 `--to original` |
| `up` | 经由各管理器自身的升级命令写 manifest/活跃二进制，另在数据根写快照历史；`--dry-run` 零写入 | 经由各管理器（它们的升级命令）；`--dry-run` 不联网 | 升级全部已知包管理器并对比升级前后的全机库存快照差异；任一管理器失败或快照持久化失败即退出码 1 |
...
- Release 存在 checksum 资产时，所选资产的条目缺失、无效或不匹配一律失败关闭——
  hukou 不会"应该没事吧"地绕过校验缺口。Release 完全没有 checksum 资产时同样
  **默认拒绝**；只有显式 `--allow-unverified` 才继续，并在 stdout/stderr 打印
  `UNVERIFIED`，同时在 manifest/事务审计中落盘 `checksum_verified: false`（加上
  算出的资产 SHA-256）。`--allow-unverified` **不能**旁路"有 checksum 但不可用"
  （缺条目/不匹配）的情况。
