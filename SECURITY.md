...
- **Release-origin integrity.** Missing or invalid checksum entries fail
  closed. A release that publishes no checksum asset at all also refuses
  install by default; operators may pass `upgrade --allow-unverified` for an
  explicit bypass that records a durable `checksum_verified: false` /
  `UNVERIFIED` audit marker (the flag never bypasses a present-but-unusable
  checksum). Asset selection is deterministic. On the default production
