...
type Entry struct {
	...
	// ChecksumVerified must not use omitempty: a false value is the durable
	// UNVERIFIED audit marker for --allow-unverified bypass installs. With
	// omitempty, encoding/json drops false entirely so on-disk JSON and the
	// transaction journal payload cannot distinguish "explicitly unverified"
	// from "field absent".
	...
	// ArchiveExe is the executable's name inside the release archive when it
	// differs from the adopted tool name. Empty means "same as the tool name"
	// (the historical behavior). It is optional in every schema version that
	// understands it.
	ArchiveExe string `json:"archive_exe,omitempty"`
	// AdoptedSHA256 is the adoption fingerprint anchor: the SHA-256 of the
	// binary at the moment it was adopted, equal by construction to the
	// immutable original backup. It never changes across upgrades/rollbacks,
	// so doctor can detect store tampering of the original backup. Optional:
	// entries adopted before this field existed have no anchor.
...
func Decode(data []byte) (*Manifest, error) {
	...
	if m.SchemaVersion == CurrentSchemaVersion {
		if err := validateDecodedV2(*m); err != nil {
			return nil, i18n.Wrapf("validate schema v2 manifest: %w", err)
		}
	}
...
func decodeManifestEnvelope(data []byte) (*manifestEnvelope, error) {
	...
	if err := decodeStrictJSON(data, &envelope); err != nil {
		return nil, i18n.Wrapf("decode manifest: %w", err)
	}
	if envelope == nil {
		return nil, i18n.Errorf("decode manifest: document must be an object")
	}
	return envelope, nil
}
...
func decodeManifestForSchema(envelope manifestEnvelope) (*Manifest, error) {
	...
	if err != nil {
		return nil, i18n.Wrapf("decode manifest schema_version: %w", err)
	}
	if schemaVersion < 0 || schemaVersion > CurrentSchemaVersion {
		return nil, i18n.Errorf("unsupported schema_version %d (current %d)", schemaVersion, CurrentSchemaVersion)
	}
	m := &Manifest{SchemaVersion: schemaVersion}
	switch schemaVersion {
	case 0, 1:
		if len(envelope.Retention) != 0 {
			return nil, i18n.Errorf("decode schema v%d manifest: v2-only top-level field %q is not allowed", schemaVersion, "retention")
		}
	...
	case CurrentSchemaVersion:
		...
		if err != nil {
			return nil, i18n.Wrapf("decode schema v2 manifest: %w", err)
		}
...
func decodeSchemaVersion(raw json.RawMessage) (int, error) {
	...
	if bytes.Equal(bytes.TrimSpace(raw), []byte("null")) {
		return 0, i18n.Errorf("must be an integer, not null")
	}
...
func decodeLegacyEntries(raw json.RawMessage, schemaVersion int) ([]Entry, error) {
	...
	if err := decodeStrictJSON(raw, &encoded); err != nil {
		return nil, i18n.Wrapf("decode schema v%d entries: %w", err, schemaVersion)
	}
	entries := make([]Entry, len(encoded))
	for i, value := range encoded {
		var legacy legacyEntry
		if err := decodeStrictJSON(value, &legacy); err != nil {
			return nil, i18n.Wrapf("decode schema v%d entry %d: %w", err, schemaVersion, i)
		}
		entries[i] = Entry{
			Name:             legacy.Name,
			Path:             legacy.Path,
...
func decodeRequiredV2Retention(raw json.RawMessage) (RetentionPolicy, error) {
	if len(raw) == 0 {
		return RetentionPolicy{}, i18n.Errorf("missing required top-level field \"retention\"")
	}
	if bytes.Equal(bytes.TrimSpace(raw), []byte("null")) {
		return RetentionPolicy{}, i18n.Errorf("top-level field \"retention\" must be an object")
	}
	var decoded decodedRetention
	if err := decodeStrictJSON(raw, &decoded); err != nil {
		return RetentionPolicy{}, i18n.Wrapf("decode retention: %w", err)
	}
	if decoded.RollbackDepth == nil {
		return RetentionPolicy{}, i18n.Errorf("retention is missing required field \"rollback_depth\"")
	}
...
func decodeRequiredV2Entries(raw json.RawMessage) ([]Entry, error) {
	if len(raw) == 0 {
		return nil, i18n.Errorf("missing required top-level field \"entries\"")
	}
	if bytes.Equal(bytes.TrimSpace(raw), []byte("null")) {
		return nil, i18n.Errorf("top-level field \"entries\" must be an array")
	}
	var encoded []json.RawMessage
	if err := decodeStrictJSON(raw, &encoded); err != nil {
		return nil, i18n.Wrapf("decode entries: %w", err)
	}
	entries := make([]Entry, len(encoded))
	for i, value := range encoded {
		if err := requireV2EntryFields(value); err != nil {
			return nil, i18n.Wrapf("entry %d: %w", err, i)
		}
		if err := decodeStrictJSON(value, &entries[i]); err != nil {
			return nil, i18n.Wrapf("decode entry %d: %w", err, i)
		}
	}
	return entries, nil
}
...
func requireV2EntryFields(raw json.RawMessage) error {
	...
	if err := json.Unmarshal(raw, &fields); err != nil {
		return i18n.Wrapf("decode required fields: %w", err)
	}
	if fields == nil {
		return i18n.Errorf("entry must be an object")
	}
	for _, name := range []string{"active_activation_id", "activations", "update_policy"} {
		value, ok := fields[name]
		if !ok {
			return i18n.Errorf("missing required field %q", name)
		}
		if (name == "activations" || name == "update_policy") && bytes.Equal(bytes.TrimSpace(value), []byte("null")) {
			return i18n.Errorf("required field %q must not be null", name)
		}
	}
	return nil
}
...
func decodeStrictJSON(data []byte, target any) error {
	...
	if err := decoder.Decode(&trailing); !errors.Is(err, io.EOF) {
		if err == nil {
			return i18n.Errorf("trailing JSON value")
		}
		return err
	}
	return nil
...
func (m *Manifest) Validate() error {
	if m == nil {
		return i18n.Errorf("nil manifest")
	}
	if m.SchemaVersion != CurrentSchemaVersion {
		return i18n.Errorf("manifest must be normalized to schema_version %d", CurrentSchemaVersion)
	}
	if m.Retention.RollbackDepth < 0 {
		return i18n.Errorf("negative manifest rollback depth")
	}
	names := make(map[string]struct{}, len(m.Entries))
	paths := make(map[string]struct{}, len(m.Entries))
	for _, entry := range m.Entries {
		if err := validateEntry(entry); err != nil {
			return i18n.Wrapf("entry %q: %w", err, entry.Name)
		}
		if _, exists := names[entry.Name]; exists {
			return i18n.Errorf("duplicate entry name %q", entry.Name)
		}
		names[entry.Name] = struct{}{}
		cleanPath := filepath.Clean(entry.Path)
		if _, exists := paths[cleanPath]; exists {
			return i18n.Errorf("duplicate entry path %q", cleanPath)
		}
...
func validateEntry(entry Entry) error {
	...
	if entry.Path == "" || !filepath.IsAbs(entry.Path) || filepath.Clean(entry.Path) != entry.Path {
		return i18n.Errorf("path must be absolute and clean")
	}
	if err := ValidateActivationTag(entry.Tag); err != nil {
		return err
	}
	if !validDigest(entry.SHA256) {
		return i18n.Errorf("invalid entry SHA-256")
	}
	if err := ValidateRepository(entry.Repo); err != nil {
		return err
	}
	if entry.ArchiveExe != "" {
		if err := ValidateArchiveExe(entry.ArchiveExe); err != nil {
			return err
		}
	}
	if entry.AdoptedSHA256 != "" && !validDigest(entry.AdoptedSHA256) {
		return errors.New("invalid adopted SHA-256")
	}
	if _, err := time.Parse(time.RFC3339, entry.AdoptedAt); err != nil {
		return i18n.Wrapf("invalid adopted_at: %w", err)
	}
	if _, err := time.Parse(time.RFC3339, entry.UpdatedAt); err != nil {
		return i18n.Wrapf("invalid updated_at: %w", err)
	}
	if (entry.AssetName == "") != (entry.AssetSHA256 == "") {
		return i18n.Errorf("asset_name and asset_sha256 must be present together")
	}
	if entry.AssetSHA256 != "" && !validDigest(entry.AssetSHA256) {
		return i18n.Errorf("invalid asset SHA-256")
	}
	if entry.ChecksumVerified && (entry.AssetName == "" || entry.AssetSHA256 == "" || entry.ChecksumAsset == "") {
		return i18n.Errorf("checksum_verified requires complete asset and checksum evidence")
	}
	if entry.ChecksumAsset != "" && entry.AssetName == "" {
		return i18n.Errorf("checksum_asset requires asset evidence")
	}
	if entry.UpdatePolicy.Mode != UpdateModeSemver && entry.UpdatePolicy.Mode != UpdateModeLegacy {
		return i18n.Errorf("invalid update mode %q", entry.UpdatePolicy.Mode)
	}
	if entry.UpdatePolicy.Channel != UpdateChannelStable && entry.UpdatePolicy.Channel != UpdateChannelPrerelease {
		return i18n.Errorf("invalid update channel %q", entry.UpdatePolicy.Channel)
	}
	...
	if entry.Retention != nil && entry.Retention.RollbackDepth < 0 {
		return i18n.Errorf("negative entry rollback depth")
	}
...
func validateEntryLineage(entry Entry) error {
	if len(entry.Activations) == 0 || entry.ActiveActivationID == "" {
		return i18n.Errorf("missing active activation lineage")
	}
	...
	for index, event := range entry.Activations {
		if err := validateDecodedActivation(event, bindings); err != nil {
			return i18n.Wrapf("activation %d: %w", err, index)
		}
		...
		if event.ParentID != "" {
			if _, exists := seen[event.ParentID]; !exists {
				return i18n.Errorf("activation %q has a missing or forward parent", event.ID)
			}
		}
		if event.RevertsID != "" {
			if _, exists := seen[event.RevertsID]; !exists {
				return i18n.Errorf("activation %q reverts a missing or forward event", event.ID)
			}
		}
	...
	if active.ID != entry.ActiveActivationID || active.Tag != entry.Tag || !strings.EqualFold(active.SHA256, entry.SHA256) {
		return i18n.Errorf("active activation does not match current tag and SHA-256")
	}
	return nil
}
...
func validateComponent(kind, value string, allowOriginal bool) error {
	if value == "" || value == "." || value == ".." || strings.Contains(value, "..") || strings.ContainsAny(value, `/\`) {
		return i18n.Errorf("invalid %s %q", kind, value)
	}
...
// ValidateArchiveExe verifies that an archive-internal executable name is safe
// to use for archive member selection. It must be a single, non-empty path
// component; the reserved backup namespace is not relevant here because the
// value never becomes a store directory component.
func ValidateArchiveExe(value string) error {
	return validateComponent("archive_exe", value, true)
}
...
func ValidateRepository(value string) error {
	...
	if len(parts) != 2 || parts[0] == "" || parts[1] == "" || strings.TrimSpace(value) != value {
		return i18n.Errorf("repository must be owner/repo")
	}
	return nil
}
...
func validateDecodedV2(m Manifest) error {
	...
	for _, entry := range m.Entries {
		if entry.UpdatePolicy.Mode != UpdateModeSemver && entry.UpdatePolicy.Mode != UpdateModeLegacy {
			return i18n.Errorf("entry %q has invalid update mode %q", entry.Name, entry.UpdatePolicy.Mode)
		}
		if entry.UpdatePolicy.Channel != UpdateChannelStable && entry.UpdatePolicy.Channel != UpdateChannelPrerelease {
			return i18n.Errorf("entry %q has invalid update channel %q", entry.Name, entry.UpdatePolicy.Channel)
		}
		if entry.Retention != nil && entry.Retention.RollbackDepth < 0 {
			return i18n.Errorf("entry %q has negative rollback depth", entry.Name)
		}
		if len(entry.Activations) == 0 || entry.ActiveActivationID == "" {
			return i18n.Errorf("entry %q has no active activation lineage", entry.Name)
		}
		seen := make(map[string]struct{}, len(entry.Activations))
		bindings := make(map[string]string, len(entry.Activations))
		for index, event := range entry.Activations {
			if err := validateDecodedActivation(event, bindings); err != nil {
				return i18n.Wrapf("entry %q activation %d: %w", err, entry.Name, index)
			}
			if _, exists := seen[event.ID]; exists {
				return i18n.Errorf("entry %q has duplicate activation id %q", entry.Name, event.ID)
			}
			if event.ParentID != "" {
				if _, exists := seen[event.ParentID]; !exists {
					return i18n.Errorf("entry %q activation %q has a missing or forward parent", entry.Name, event.ID)
				}
			}
			if event.RevertsID != "" {
				if _, exists := seen[event.RevertsID]; !exists {
					return i18n.Errorf("entry %q activation %q reverts a missing or forward event", entry.Name, event.ID)
				}
			}
			seen[event.ID] = struct{}{}
		}
		active := entry.Activations[len(entry.Activations)-1]
		if active.ID != entry.ActiveActivationID || active.Tag != entry.Tag || !strings.EqualFold(active.SHA256, entry.SHA256) {
			return i18n.Errorf("entry %q active activation does not match current tag and SHA-256", entry.Name)
		}
	}
	return nil
}
...
func validateDecodedActivation(event ActivationEvent, bindings map[string]string) error {
	if event.ID == "" || len(event.ID) > 128 {
		return i18n.Errorf("invalid activation id")
	}
	for _, character := range event.ID {
		...
		return i18n.Errorf("activation id contains an unsupported character")
	}
	switch event.Operation {
	case "legacy", "adopt", "upgrade", "rollback", "repair":
	default:
		return i18n.Errorf("unsupported operation %q", event.Operation)
	}
	if err := ValidateActivationTag(event.Tag); err != nil {
		return err
	}
	if len(event.SHA256) != sha256.Size*2 {
		return i18n.Errorf("invalid activation SHA-256")
	}
	...
	if boundSHA, exists := bindings[event.Tag]; exists && boundSHA != normalizedSHA {
		return i18n.Errorf("activation tag %q is already bound to a different SHA-256", event.Tag)
	}
	bindings[event.Tag] = normalizedSHA
	if _, err := time.Parse(time.RFC3339, event.ActivatedAt); err != nil {
		return i18n.Wrapf("invalid activation timestamp: %w", err)
	}
	return nil
}
...
func (m *Manifest) Normalize() error {
	...
	if m.SchemaVersion < 0 || m.SchemaVersion > CurrentSchemaVersion {
		return i18n.Errorf("unsupported schema_version %d (current %d)", m.SchemaVersion, CurrentSchemaVersion)
	}
	if m.Entries == nil {
...
func (m *Manifest) save(path string, fs durableOperations) error {
	...
	if err := requireRegularOrMissing(backupPath); err != nil {
		return i18n.Wrapf("validate manifest backup: %w", err)
	}

	previous, previousInfo, err := readRegularFile(path)
	if err != nil && !errors.Is(err, os.ErrNotExist) {
		return i18n.Wrapf("read current manifest: %w", err)
	}
	if err == nil {
		if _, err := Decode(previous); err != nil {
			return i18n.Wrapf("current manifest is not valid; refusing to replace it: %w", err)
		}
		if err := fs.AtomicWriteFile(backupPath, previous, 0o600); err != nil {
			return i18n.Wrapf("preserve previous manifest: %w", err)
		}
		currentInfo, statErr := os.Lstat(path)
		if statErr != nil {
			return i18n.Wrapf("recheck current manifest: %w", statErr)
		}
		if !currentInfo.Mode().IsRegular() || currentInfo.Mode()&os.ModeSymlink != 0 || !os.SameFile(previousInfo, currentInfo) {
			return i18n.Errorf("current manifest changed while saving: %s", path)
		}
	}

	if err := fs.AtomicWriteFile(path, encoded.Bytes(), 0o600); err != nil {
		return i18n.Wrapf("write manifest: %w", err)
	}
	return nil
}
...
func readRegularFile(path string) ([]byte, os.FileInfo, error) {
	...
	if before.Mode()&os.ModeSymlink != 0 {
		return nil, nil, i18n.Errorf("manifest must not be a symlink: %s", path)
	}
	if !before.Mode().IsRegular() {
		return nil, nil, i18n.Errorf("manifest is not a regular file: %s", path)
	}
	...
	if !after.Mode().IsRegular() || !os.SameFile(before, after) {
		return nil, nil, i18n.Errorf("manifest changed while opening: %s", path)
	}
...
func requireRegularOrMissing(path string) error {
	...
	if info.Mode()&os.ModeSymlink != 0 {
		return i18n.Errorf("path must not be a symlink: %s", path)
	}
	if !info.Mode().IsRegular() {
		return i18n.Errorf("path is not a regular file: %s", path)
	}
	return nil
}
