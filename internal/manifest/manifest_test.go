...
func TestLegacyEntryLoadsWithoutOptionalAssetFields(t *testing.T) {
	...
	// asset_name / asset_sha256 / checksum_asset remain omitempty optional
	// provenance. checksum_verified is intentionally always written so an
	// explicit false (UNVERIFIED bypass) cannot be confused with field absence.
	for _, field := range []string{"asset_name", "asset_sha256", "checksum_asset"} {
		if strings.Contains(string(data), `"`+field+`"`) {
			t.Fatalf("zero-value optional field %q must be omitted: %s", field, data)
		}
	}
	if !strings.Contains(string(data), `"checksum_verified": false`) {
		t.Fatalf("checksum_verified=false must appear in serialized JSON (not omitempty): %s", data)
	}
}

// TestChecksumVerifiedSerializedInJSONRaw asserts the on-disk / journal wire
// form, not merely the decoded Go zero value. With omitempty on a bool, false
// is dropped from JSON entirely; these checks fail on that unfixed shape.
func TestChecksumVerifiedSerializedInJSONRaw(t *testing.T) {
	dir := t.TempDir()

	verified := manifest.PrepareEntry(fixtureEntry())
	verified.ChecksumVerified = true
	unverified := manifest.PrepareEntry(fixtureEntry())
	unverified.Name = "unverified-bin"
	unverified.Path = "/usr/local/bin/unverified-bin"
	unverified.ChecksumAsset = ""
	unverified.ChecksumVerified = false

	m := &manifest.Manifest{
		SchemaVersion: manifest.CurrentSchemaVersion,
		Retention:     manifest.DefaultRetentionPolicy(),
		Entries:       []manifest.Entry{verified, unverified},
	}
	path := filepath.Join(dir, "manifest.json")
	if err := m.Save(path); err != nil {
		t.Fatalf("Save: %v", err)
	...
	// Indent encoder emits a space after the colon (same as Save / encodeManifest).
	if !strings.Contains(raw, `"checksum_verified": true`) {
		t.Fatalf("verified path JSON must contain checksum_verified true, got:\n%s", raw)
	}
	if !strings.Contains(raw, `"checksum_verified": false`) {
		t.Fatalf("unverified path JSON must contain checksum_verified false (omitempty would swallow it), got:\n%s", raw)
	}

	// Re-encode through the same indented shape as Save / encodeManifest (journal payload).
	encData, err := json.MarshalIndent(m, "", "  ")
	if err != nil {
		t.Fatal(err)
	}
	encRaw := string(encData)
	if !strings.Contains(encRaw, `"checksum_verified": true`) {
		t.Fatalf("MarshalIndent verified marker missing:\n%s", encRaw)
	}
	if !strings.Contains(encRaw, `"checksum_verified": false`) {
		t.Fatalf("MarshalIndent unverified marker missing:\n%s", encRaw)
	}

	// Round-trip decode still recovers both states.
	loaded, err := manifest.Load(path)
	if err != nil {
		t.Fatal(err)
	}
	if got := loaded.Get("mybin"); got == nil || !got.ChecksumVerified {
		t.Fatalf("decoded verified entry: %+v", got)
	}
	if got := loaded.Get("unverified-bin"); got == nil || got.ChecksumVerified {
		t.Fatalf("decoded unverified entry: %+v", got)
	}
}
...
func TestValidateArchiveExe(t *testing.T) {
	for _, bad := range []string{"", "a/b", "a\\b", "..", ".", "a..b"} {
		if err := manifest.ValidateArchiveExe(bad); err == nil {
			t.Fatalf("ValidateArchiveExe(%q): expected rejection", bad)
		}
	}
	for _, ok := range []string{"rg", "my-tool", "我的工具", "original"} {
		if err := manifest.ValidateArchiveExe(ok); err != nil {
			t.Fatalf("ValidateArchiveExe(%q): unexpected error: %v", ok, err)
		}
	}
}

func TestEntryArchiveExeRoundTripAndValidation(t *testing.T) {
	entry := manifest.PrepareEntry(fixtureEntry())
	entry.ArchiveExe = "realtool"
	candidate := &manifest.Manifest{
		SchemaVersion: manifest.CurrentSchemaVersion,
		Retention:     manifest.DefaultRetentionPolicy(),
		Entries:       []manifest.Entry{entry},
	...
	requireValidateAndDecodeAccepted(t, candidate)
	doc, err := json.Marshal(candidate)
	if err != nil {
		t.Fatal(err)
	}
	m, err := manifest.Decode(doc)
	if err != nil {
		t.Fatalf("decode entry with archive_exe: %v", err)
	}
	if got := m.Get(entry.Name).ArchiveExe; got != "realtool" {
		t.Fatalf("archive_exe lost in round-trip: %q", got)
	}

	bad := manifest.PrepareEntry(fixtureEntry())
	bad.ArchiveExe = "dir/name"
	badCandidate := &manifest.Manifest{
		SchemaVersion: manifest.CurrentSchemaVersion,
		Retention:     manifest.DefaultRetentionPolicy(),
		Entries:       []manifest.Entry{bad},
	}
	requireValidateAndDecodeRejected(t, badCandidate)
}

func TestEntryAdoptedSHA256Validation(t *testing.T) {
	entry := manifest.PrepareEntry(fixtureEntry())
	entry.AdoptedSHA256 = strings.Repeat("ab", 32)
	candidate := &manifest.Manifest{
		SchemaVersion: manifest.CurrentSchemaVersion,
		Retention:     manifest.DefaultRetentionPolicy(),
		Entries:       []manifest.Entry{entry},
	...
	bad := manifest.PrepareEntry(fixtureEntry())
	bad.AdoptedSHA256 = "not-a-digest"
	badCandidate := &manifest.Manifest{
		SchemaVersion: manifest.CurrentSchemaVersion,
		Retention:     manifest.DefaultRetentionPolicy(),
		Entries:       []manifest.Entry{bad},
