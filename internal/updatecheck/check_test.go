...
func TestCheckerAppendsMajorVersionWarning(t *testing.T) {
	entry := updateEntry(t, "v1.0.0")
	src := &recordingReleaseSource{
		releases: []ghrelease.Release{
			{TagName: "v2.0.0", Assets: []ghrelease.Asset{completeAsset(platformAsset(t))}},
		},
	}
	checked, err := New(src).Check(entry, "")
	if err != nil {
		t.Fatal(err)
	}
	if checked.Status != StatusOutdated {
		t.Fatalf("status = %s", checked.Status)
	}
	if !strings.Contains(checked.Note, "major version jump") {
		t.Fatalf("major warning missing from note: %q", checked.Note)
	}

	// Minor bump: no warning.
	src2 := &recordingReleaseSource{
		releases: []ghrelease.Release{
			{TagName: "v1.9.0", Assets: []ghrelease.Asset{completeAsset(platformAsset(t))}},
		},
	}
	entry2 := updateEntry(t, "v1.8.0")
	checked2, err := New(src2).Check(entry2, "")
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(checked2.Note, "major version jump") {
		t.Fatalf("false major warning: %q", checked2.Note)
	}
}
...
func platformAsset(t *testing.T) string {
	...
	switch runtime.GOOS {
	case "darwin":
		return "tool-v2.0.0-" + suffix + ".tar.gz"
	default:
		return "tool-v2.0.0-" + suffix + ".tar.gz"
	}
}
