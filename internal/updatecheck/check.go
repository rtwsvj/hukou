...
func (c Checker) Check(entry manifest.Entry, assetFilter string) (Checked, error) {
	...
	if err != nil {
		...
		return checked, i18n.Wrapf("read current file: %w", err)
	}
	if currentSHA != entry.SHA256 {
		checked.Status = StatusDrifted
		checked.Error = "current file sha256 does not match manifest"
		return checked, i18n.Errorf("current file sha256 does not match manifest (possible external modification)")
	}
	...
	if !ok {
		...
		return checked, i18n.Errorf("invalid repo %q", entry.Repo)
	}
	if c.Releases == nil {
		checked.Status = StatusUnavailable
		checked.Error = "release source is not configured"
		return checked, i18n.Errorf("release source is not configured")
	}
	...
	if err := store.ValidateTag(decision.CandidateTag); err != nil {
		...
		return checked, i18n.Wrapf("selected release tag %q cannot be stored: %w", err, decision.CandidateTag)
	}
	...
	if versionpolicy.IsMajorUpgrade(entry.Tag, decision.CandidateTag) {
		if checked.Note != "" {
			checked.Note += "; "
		}
		checked.Note += i18n.T("major version jump (breaking changes likely)")
	}
	return checked, nil
}
...
func (c Checker) selectRelease(entry manifest.Entry, owner, repo string) (ghrelease.Release, versionpolicy.Decision, error) {
	...
	if policy.PinnedTag != "" {
		...
		if !ok {
			return ghrelease.Release{}, versionpolicy.Decision{}, i18n.Errorf("release source does not support exact tag lookup required by pin %q", policy.PinnedTag)
		}
	...
	return ghrelease.Release{}, versionpolicy.Decision{}, i18n.Errorf("selected release %q is missing from metadata response", decision.CandidateTag)
}
