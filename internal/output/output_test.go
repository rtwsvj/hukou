...
// Fix #8: truncate must not split multi-byte UTF-8 runes. The display-width
// variant applies the same rune-safety with wide-rune column accounting.
func TestTruncate_utf8Safe(t *testing.T) {
	// Each Chinese rune is 3 bytes in UTF-8 and 2 display columns.
	s := "你好世界测试字符串内容更多"
	// max=10 display columns: body budget 7 → at most 3 runes (6 cols) + "..."
	got := TruncateDisplay(s, 10)
	...
	if DisplayWidth(got) > 10 {
		t.Fatalf("display width %d > 10: %q", DisplayWidth(got), got)
	}
	...
	if TruncateDisplay("abc", 3) != "abc" {
		t.Fatal("short string should pass through")
	}
	// Single multi-byte at start with tiny budget
	got = TruncateDisplay("世界", 4)
	if !utf8.ValidString(got) {
		t.Fatalf("invalid: %q", got)
	}
	// Wide runes consume display columns, not bytes.
	if got := TruncateDisplay("hello世界", 7); got != "hell..." {
		t.Fatalf("wide budget mismatch: %q", got)
	}
}

func TestWriteSummaryLineEndsWithNewline(t *testing.T) {
	var buf bytes.Buffer
	r := Report{Summary: Summary{Total: 2, SourceN: 1, Unknown: 0, Shadowed: 0, Sources: map[string]int{"unknown": 2}}}
	if err := WriteSummaryLine(&buf, r); err != nil {
		t.Fatal(err)
	}
	out := buf.String()
	if !strings.HasSuffix(out, "\n") {
		t.Fatalf("summary output missing trailing newline: %q", out)
	}
	if !strings.Contains(out, "by source: unknown=2") {
