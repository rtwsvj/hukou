...
func (FileSystem) SyncFile(file *os.File) error {
	if file == nil {
		return i18n.Errorf("sync file: nil file")
	}
	if err := file.Sync(); err != nil {
		return i18n.Wrapf("sync file %s: %w", err, file.Name())
	}
	return nil
}
...
func (fs FileSystem) SyncDir(path string) error {
	...
	if err != nil {
		return i18n.Wrapf("open directory %s for sync: %w", err, path)
	}
	info, statErr := dir.Stat()
	if statErr == nil && !info.IsDir() {
		statErr = i18n.Errorf("path is not a directory: %s", path)
	}
	var syncErr error
	if statErr == nil {
		syncErr = dir.Sync()
		if syncErr != nil {
			syncErr = i18n.Wrapf("sync directory %s: %w", syncErr, path)
		}
	}
	closeErr := dir.Close()
	if closeErr != nil {
		closeErr = i18n.Wrapf("close directory %s: %w", closeErr, path)
	}
...
func (fs FileSystem) Mkdir(path string, mode os.FileMode) error {
	...
	if err := fs.SyncParent(path); err != nil {
		return i18n.Wrapf("persist directory %s: %w", err, path)
	}
	return nil
}
...
func (fs FileSystem) MkdirAll(path string, mode os.FileMode) error {
	...
	for {
		...
		if err == nil {
			if !info.IsDir() {
				return &os.PathError{Op: "mkdir", Path: current, Err: i18n.Errorf("not a directory")}
			}
			break
		}
	...
	if err := fs.syncAncestorEntries(current); err != nil {
		return i18n.Wrapf("persist existing directory ancestry for %s: %w", err, clean)
	}
	...
	for i := len(missing) - 1; i >= 0; i-- {
		if err := fs.Mkdir(missing[i], mode); err != nil {
			...
			if !info.IsDir() {
				return &os.PathError{Op: "mkdir", Path: missing[i], Err: i18n.Errorf("not a directory")}
			}
			if syncErr := fs.SyncParent(missing[i]); syncErr != nil {
				return i18n.Wrapf("persist concurrently created directory %s: %w", syncErr, missing[i])
			}
		}
	}
	return nil
...
func (fs FileSystem) AtomicWriteFile(path string, data []byte, mode os.FileMode) (retErr error) {
	...
	defer func() {
		...
		if removeErr := fs.Remove(tmpPath); removeErr != nil && !errors.Is(removeErr, os.ErrNotExist) {
			retErr = errors.Join(retErr, i18n.Wrapf("remove temporary file %s: %w", removeErr, tmpPath))
		}
	}()
...
func (fs FileSystem) Rename(oldPath, newPath string) error {
	...
	if oldDir != newDir {
		return i18n.Errorf("durable rename requires the same parent directory: %s -> %s", oldPath, newPath)
	}
	if err := os.Rename(oldPath, newPath); err != nil {
		return err
	}
	if err := fs.SyncDir(newDir); err != nil {
		return i18n.Wrapf("persist rename %s -> %s: %w", err, oldPath, newPath)
	}
	return nil
}
...
func (fs FileSystem) Link(oldPath, newPath string) error {
	...
	if err := fs.SyncParent(newPath); err != nil {
		return i18n.Wrapf("persist link %s -> %s: %w", err, newPath, oldPath)
	}
	return nil
}
...
func (fs FileSystem) Remove(path string) error {
	...
	if err := fs.SyncParent(path); err != nil {
		return i18n.Wrapf("persist removal of %s: %w", err, path)
	}
	return nil
}
...
func (fs FileSystem) RemoveAll(path string) error {
	if _, err := os.Lstat(path); err != nil {
		if errors.Is(err, os.ErrNotExist) {
			...
			if syncErr := fs.SyncParent(path); syncErr != nil {
				return i18n.Wrapf("persist absence of %s: %w", syncErr, path)
			}
			return nil
		}
		return err
	...
	if err := fs.SyncParent(path); err != nil {
		return i18n.Wrapf("persist recursive removal of %s: %w", err, path)
	}
	return nil
}
