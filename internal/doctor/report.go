...
type Finding struct {
	...
	// Message is the canonical English rendering and the stable JSON value:
	// the machine contract requires doctor --json to be byte-identical in
	// every locale. Localization happens only in the text renderer, which
	// re-renders from MessageTemplate with the locale of the moment.
...
func (r *Report) add(severity Severity, code, scope, subject, path, format string, args ...any) {
	// The message template is localized here (not at render time) because the
	// rendered message already has its arguments substituted and would never
	// match a catalog key again.
	r.Findings = append(r.Findings, Finding{
		...
		Subject:         subject,
		Path:            path,
		Message:         fmt.Sprintf(format, args...), // canonical English for JSON
