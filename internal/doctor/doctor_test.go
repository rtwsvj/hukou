...
func TestAdoptAnchorMismatchDetected(t *testing.T) {
	// Healthy with a matching anchor: original bytes == anchor.
	root, entry := validAdoptState(t)
	entry.AdoptedSHA256 = hashString("original")
	saveEntryAnchor(t, root, entry)
	if report := doctor.Scan(doctor.Options{DataRoot: root}); !report.Healthy() {
		t.Fatalf("matching anchor should be healthy: %+v", report.Findings)
	}

	// Tampered original: the store's original no longer hashes to the anchor.
	tamperRoot, tamperEntry := validAdoptState(t)
	tamperEntry.AdoptedSHA256 = hashString("original")
	writeExecutable(t, filepath.Join(tamperRoot, "store", "tool", "original", "tool"), "tampered")
	saveEntryAnchor(t, tamperRoot, tamperEntry)
	report := doctor.Scan(doctor.Options{DataRoot: tamperRoot})
	if report.Healthy() {
		t.Fatal("tampered original must be detected")
	}
	if !hasCode(report, "ADOPT_ANCHOR_MISMATCH") {
		t.Fatalf("anchor finding missing: %+v", report.Findings)
	}

	// Legacy entries without an anchor are not flagged.
	legacyRoot, _ := validAdoptState(t)
	if report := doctor.Scan(doctor.Options{DataRoot: legacyRoot}); report.Healthy() == false {
		t.Fatalf("legacy entry without anchor should stay healthy: %+v", report.Findings)
	}
}

func saveEntryAnchor(t *testing.T, root string, entry manifest.Entry) {
	t.Helper()
	m, err := manifest.Load(filepath.Join(root, "manifest.json"))
	if err != nil {
		t.Fatal(err)
	}
	m.Put(manifest.PrepareEntry(entry))
	if err := m.Save(filepath.Join(root, "manifest.json")); err != nil {
		t.Fatal(err)
	}
}
