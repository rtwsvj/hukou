...
var (
	ErrInvalidPolicy         = i18n.Errorf("invalid update policy")
	ErrNoCandidate           = i18n.Errorf("no eligible release candidate")
	ErrPinnedReleaseNotFound = i18n.Errorf("pinned release not found")
	ErrCurrentNotSemver      = i18n.Errorf("current version is not semantic")
	ErrDowngrade             = i18n.Errorf("release candidate would downgrade the current version")
)
...
func Normalize(policy manifest.UpdatePolicy) (manifest.UpdatePolicy, error) {
	...
	default:
		return manifest.UpdatePolicy{}, i18n.Wrapf("%w: unsupported mode %q", ErrInvalidPolicy, policy.Mode)
	}
	switch policy.Channel {
	case manifest.UpdateChannelStable, manifest.UpdateChannelPrerelease:
	default:
		return manifest.UpdatePolicy{}, i18n.Wrapf("%w: unsupported channel %q", ErrInvalidPolicy, policy.Channel)
	}
	if policy.PinnedTag != "" && strings.TrimSpace(policy.PinnedTag) != policy.PinnedTag {
		return manifest.UpdatePolicy{}, i18n.Wrapf("%w: pinned tag has surrounding whitespace", ErrInvalidPolicy)
	}
	return policy, nil
}
...
func selectPin(pin, currentTag string, releases []Release) (Decision, error) {
	if pin == currentTag {
		return Decision{
			...
			CandidateTag: pin,
			Pinned:       true,
			Reason:       i18n.T("exact pin is already active"),
		}, nil
	}
	for _, release := range releases {
		...
		if release.Draft {
			return Decision{}, i18n.Wrapf("%w: %q is a draft", ErrPinnedReleaseNotFound, pin)
		}
		return Decision{
			...
			Pinned:       true,
			Reason:       i18n.T("activate exact pinned release"),
		}, nil
	}
	return Decision{}, i18n.Wrapf("%w: %q", ErrPinnedReleaseNotFound, pin)
}
...
func selectSemver(channel manifest.UpdateChannel, currentTag string, releases []Release) (Decision, error) {
	...
	if unorderedBaseline(currentTag) {
		return Decision{
			...
			Reason:       i18n.T("establish semantic version baseline"),
		}, nil
	}
	currentVersion := normalizeVersion(currentTag)
	if !IsSemanticTag(currentTag) {
		return Decision{}, i18n.Wrapf("%w: %q", ErrCurrentNotSemver, currentTag)
	}
	switch semver.Compare(selectedVersion, currentVersion) {
	case -1:
		return Decision{}, i18n.Wrapf("%w: candidate %s is lower than current %s", ErrDowngrade, selected.Tag, currentTag)
	case 0:
		return Decision{
			Action:       ActionNone,
			CurrentTag:   currentTag,
			CandidateTag: selected.Tag,
			Reason:       i18n.T("highest eligible semantic version is already active"),
		}, nil
	default:
		return Decision{
			Action:       ActionActivate,
			CurrentTag:   currentTag,
			CandidateTag: selected.Tag,
			Reason:       i18n.T("higher eligible semantic version is available"),
		}, nil
	}
}
...
// IsMajorUpgrade reports whether moving from the current tag to the latest tag
// crosses a semantic major version boundary (x.Y.Z -> y.Y.Z with y > x).
// Non-semantic tags on either side return false — the warning is deliberately
// only as strong as the version data.
