...
func WriteFile(path string, report Report) error {
	if path == "" {
		return i18n.Errorf("support bundle output path is required")
	}
	...
	if !parentInfo.IsDir() {
		return i18n.Errorf("support bundle output parent is not a directory")
	}
	if info, err := os.Lstat(path); err == nil {
		if info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() {
			return i18n.Errorf("support bundle output must be a regular file or missing")
		}
