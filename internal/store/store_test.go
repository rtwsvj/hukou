...
func TestValidateNameRejectsReservedNames(t *testing.T) {
	for _, bad := range []string{"original", "Original", "ORIGINAL", ".tmp", ".TMP"} {
		if err := store.ValidateName(bad); err == nil {
			t.Fatalf("ValidateName(%q): expected rejection", bad)
		}
	}
	for _, ok := range []string{"notoriginal", "tool-original", "original-tool"} {
		if err := store.ValidateName(ok); err != nil {
			t.Fatalf("ValidateName(%q): unexpected error: %v", ok, err)
		}
	}
}
...
func TestValidateNameRejectsWhitespaceAndControlChars(t *testing.T) {
	for _, bad := range []string{"   ", "\t", "\n", "a\nb", "a\tb", "a\x1bb", "a\x7fb"} {
		if err := store.ValidateName(bad); err == nil {
			t.Fatalf("ValidateName(%q): expected rejection", bad)
		}
		if err := store.ValidateTag(bad); err == nil {
			t.Fatalf("ValidateTag(%q): expected rejection", bad)
		}
	}
	for _, ok := range []string{"hello world", "我的工具", "🦀crab", "-rf", "~", "*.txt"} {
		if err := store.ValidateName(ok); err != nil {
			t.Fatalf("ValidateName(%q): unexpected error: %v", ok, err)
		}
	}
