...
func (s *Store) ActivationSource(name, tag string) (string, error) {
	...
	if err != nil {
		return "", i18n.Wrapf("activation source: %w", err)
	}
	return resolved, nil
}
...
func (s *Store) PrepareOriginalPath(name, binaryName string) (string, error) {
	...
	if binaryName == "" || filepath.Base(binaryName) != binaryName {
		return "", i18n.Errorf("invalid original binary name %q", binaryName)
	}
...
func (s *Store) PreflightOriginalPath(name, binaryName string) (string, error) {
	...
	if _, err := s.storeDir(false, ".tmp"); err != nil && !errors.Is(err, os.ErrNotExist) {
		return "", i18n.Wrapf("validate staging namespace: %w", err)
	}
	...
	if len(entries) != 0 {
		return "", i18n.Errorf("original backup namespace is not empty: %s", originalDir)
	}
...
func SnapshotLive(path string) (*LiveSnapshot, error) {
	...
	if !info.Mode().IsRegular() {
		return nil, i18n.Errorf("cannot snapshot non-regular live path: %s", path)
	}
...
func (s *LiveSnapshot) Restore() error {
	...
	if s.backupPath == "" {
		return i18n.Errorf("regular-file snapshot has no backup")
	}
	if err := durablefs.Rename(s.backupPath, s.path); err != nil {
		return i18n.Wrapf("restore live path: %w", err)
	}
	s.done = true
	return nil
}
