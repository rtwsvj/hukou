// Package assetpick selects the right release asset for the current platform.
//
// This file is adapted from eget (github.com/zyedidia/eget, detect.go at
// commit 0983dea), Copyright (c) 2021: Zachary Yedidia, licensed under the
// MIT License. See LICENSES/eget-MIT.txt for the full license text.
//
// Modifications for hukou: package rename; fixed the priority-branch error
// message to report len(priority) instead of len(matches); no logic changes.
// Tiebreak extensions (darwin/arm64 fallback, extension preference, blacklist)
// live in separate files of this package, layered on top of this base.
package assetpick
...
func (dc *DetectorChain) Detect(assets []string) (string, []string, error) {
	...
	return "", assets, i18n.Errorf("%d candidates found for asset chain", len(assets))
}
...
func (a *AllDetector) Detect(assets []string) (string, []string, error) {
	...
	return "", assets, i18n.Errorf("%d matches found", len(assets))
}
...
func (s *SingleAssetDetector) Detect(assets []string) (string, []string, error) {
	...
	if len(candidates) == 1 {
		...
		return "", candidates, i18n.Errorf("%d candidates found for asset `%s`", len(candidates), s.Asset)
	}
	return "", nil, i18n.Errorf("asset `%s` not found", s.Asset)
}
...
func NewSystemDetector(sos, sarch string) (*SystemDetector, error) {
	os, ok := goosmap[sos]
	if !ok {
		return nil, i18n.Errorf("unsupported target OS: %s", sos)
	}
	arch, ok := goarchmap[sarch]
	if !ok {
		return nil, i18n.Errorf("unsupported target arch: %s", sarch)
	}
...
func (d *SystemDetector) Detect(assets []string) (string, []string, error) {
	...
	if len(priority) == 1 {
		...
		return "", priority, i18n.Errorf("%d priority matches found", len(priority))
	} else if len(matches) == 1 {
		return matches[0], nil, nil
	} else if len(matches) > 1 {
		return "", matches, i18n.Errorf("%d matches found", len(matches))
	} else if len(candidates) == 1 {
		return candidates[0], nil, nil
	} else if len(candidates) > 1 {
		return "", candidates, i18n.Errorf("%d candidates found (unsure architecture)", len(candidates))
	} else if len(all) == 1 {
		return all[0], nil, nil
	}
	return "", all, i18n.Errorf("no candidates found")
}
