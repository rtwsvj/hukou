...
func TestPickMuslPreferenceOnLinux(t *testing.T) {
	assets := []string{
		"tool-v1.0.0-x86_64-unknown-linux-gnu.tar.gz",
		"tool-v1.0.0-x86_64-unknown-linux-musl.tar.gz",
	}
	chosen, note, err := Pick(assets, "linux", "amd64", "")
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(chosen, "musl") {
		t.Fatalf("expected musl preference, got %q", chosen)
	}
	if !strings.Contains(note, "musl-preference") {
		t.Fatalf("musl note missing: %q", note)
	}
	// darwin: musl filter must be a no-op (both candidates would otherwise
	// remain and fail closed as ambiguous).
	_, _, err = Pick([]string{"tool-x86_64-linux-gnu.tar.gz", "tool-x86_64-linux-musl.tar.gz"}, "darwin", "amd64", "")
	if err == nil {
		t.Fatal("darwin should not match linux-only assets; expected no-candidate error")
	}
}

func TestPickDeterministicAcrossInputOrders(t *testing.T) {
	set := []string{
	...
	for i := 0; i < 5; i++ {
		shuffled := append([]string(nil), set...)
		if i%2 == 1 {
			shuffled[0], shuffled[1] = shuffled[1], shuffled[0]
		}
		chosen, _, err := Pick(shuffled, "darwin", "arm64", "")
		if err != nil {
			t.Fatal(err)
		}
		results[chosen] = true
	...
	if len(results) != 1 {
		t.Fatalf("selection depends on input order: %v", results)
	}
}
