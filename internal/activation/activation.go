...
var ErrNoPreviousActivation = errors.New("no previous activation")
...
func RecordAdopt(entry *manifest.Entry, eventID, activatedAt string) error {
	if entry == nil {
		return errors.New("nil manifest entry")
	}
	if entry.ActiveActivationID != "" || len(entry.Activations) != 0 {
		return errors.New("adopt requires an empty activation lineage")
	}
...
func RecordUpgrade(entry *manifest.Entry, eventID, tag, sha256, activatedAt string) error {
	...
	if err := Validate(*entry); err != nil {
		return fmt.Errorf("validate current activation lineage: %w", err)
	}
	...
	if _, ok := eventIndex(*entry)[eventID]; ok {
		return fmt.Errorf("activation id %q already exists", eventID)
	}
	event := manifest.ActivationEvent{
		ID:          eventID,
...
func FindAncestorByTag(entry manifest.Entry, tag string) (manifest.ActivationEvent, error) {
	if tag == "" {
		return manifest.ActivationEvent{}, errors.New("empty activation tag")
	}
	...
	return manifest.ActivationEvent{}, fmt.Errorf("tag %q is not an activation ancestor", tag)
}
...
func RecordRollback(entry *manifest.Entry, eventID, targetID, activatedAt string) error {
	...
	if !found {
		return fmt.Errorf("activation %q is not an ancestor of the current version", targetID)
	}
	event := manifest.ActivationEvent{
		ID:          eventID,
...
func Ancestors(entry manifest.Entry, limit int) ([]manifest.ActivationEvent, error) {
	if limit < 0 {
		return nil, errors.New("negative ancestor limit")
	}
	if err := Validate(entry); err != nil {
		return nil, err
	}
...
func Validate(entry manifest.Entry) error {
	if len(entry.Activations) == 0 {
		return errors.New("activation lineage is empty")
	}
	if entry.ActiveActivationID == "" {
		return errors.New("active activation id is empty")
	}
	...
	for i, event := range entry.Activations {
		if err := validateEventInput(event.ID, event.Tag, event.SHA256, event.ActivatedAt); err != nil {
			return fmt.Errorf("activation %d: %w", i, err)
		}
		if err := bindTagSHA(bindings, event.Tag, event.SHA256); err != nil {
			return fmt.Errorf("activation %d: %w", i, err)
		}
		if !knownOperation(event.Operation) {
			return fmt.Errorf("activation %q has unsupported operation %q", event.ID, event.Operation)
		}
		...
		if event.ParentID != "" {
			if _, exists := index[event.ParentID]; !exists {
				return fmt.Errorf("activation %q has missing or forward parent %q", event.ID, event.ParentID)
			}
		}
		if event.RevertsID != "" {
			if _, exists := index[event.RevertsID]; !exists {
				return fmt.Errorf("activation %q reverts missing or forward event %q", event.ID, event.RevertsID)
			}
		}
		index[event.ID] = i
	}
	...
	if !exists {
		return fmt.Errorf("active activation %q does not exist", entry.ActiveActivationID)
	}
	if activeIndex != len(entry.Activations)-1 {
		return fmt.Errorf("active activation %q is not the latest event", entry.ActiveActivationID)
	}
	active := entry.Activations[activeIndex]
	if active.Tag != entry.Tag || !strings.EqualFold(active.SHA256, entry.SHA256) {
		return fmt.Errorf("active activation %q does not match entry tag/SHA-256", active.ID)
	}
	return nil
}
...
func validateEventInput(eventID, tag, sha256, activatedAt string) error {
	...
	if !validSHA256(sha256) {
		return fmt.Errorf("invalid activation SHA-256 %q", sha256)
	}
...
func bindTagSHA(bindings map[string]string, tag, sha256 string) error {
	...
	if boundSHA, exists := bindings[tag]; exists && boundSHA != normalizedSHA {
		return fmt.Errorf("activation tag %q is already bound to a different SHA-256", tag)
	}
...
func validateID(id string) error {
	if id == "" {
		return errors.New("empty activation id")
	}
	if len(id) > 128 {
		return errors.New("activation id exceeds 128 bytes")
	}
	for _, r := range id {
		if (r >= 'a' && r <= 'z') || (r >= 'A' && r <= 'Z') ||
			(r >= '0' && r <= '9') || r == '-' || r == '_' || r == '.' {
		...
		return fmt.Errorf("activation id %q contains an unsupported character", id)
	}
	return nil
}
...
func validateTimestamp(value string) error {
	if value == "" {
		return errors.New("empty activation timestamp")
	}
	if _, err := time.Parse(time.RFC3339, value); err != nil {
		return fmt.Errorf("invalid activation timestamp %q: %w", value, err)
	}
	return nil
}
