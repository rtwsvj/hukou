...
func TestUpgradeDryRunRefusesPendingJournalWithoutRecovering(t *testing.T) {
	...
	output.Reset()
	err = doUpgrade(&output, &output, []string{"dry-run-tool"}, false, true, "", ghrelease.New(""), false)
...
func TestUpgradeAllStopsBeforeNextNetworkRequestWhenTransactionRemainsPending(t *testing.T) {
	...
	alphaChecksum := fmt.Sprintf("%x  %s\n", sha256.Sum256(alphaAsset), alphaAssetName)
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/assets/checksums.txt":
			_, _ = w.Write([]byte(alphaChecksum))
		default:
			http.NotFound(w, r)
		}
	}))
	...
	saveCalls := 0
	output.Reset()
	err := doUpgradeWithSave(&output, &output, nil, true, false, "", testGHClient(server), false, func(m *manifest.Manifest) error {
		saveCalls++
		if err := saveManifest(m); err != nil {
			return err
		}
