...
func doSupportBundle(stdout io.Writer, root, output, format string, build supportbundle.Build) error {
	if output == "" && format == "" {
		return i18n.Errorf("use exactly one of --output FILE or --format json")
	}
	if output != "" && format != "" {
		return i18n.Errorf("--output and --format are mutually exclusive")
	}
	if format != "" && format != "json" {
		return i18n.Errorf("unsupported support bundle format %q (expected json)", format)
	}
	...
	if output != "" {
		...
		_, err := fmt.Fprintf(stdout, "%s\n", i18n.T("Support bundle written: %s", output))
		return err
	}
