...
func doUpHistory(stdout io.Writer, snapsDir string, asJSON bool) error {
	...
	if len(entries) == 0 {
		_, err := fmt.Fprintln(stdout, "no up runs recorded")
		return err
	}
...
func writeHistoryTable(w io.Writer, entries []upHistoryEntryJSON) error {
	...
	fmt.Fprintln(tw, "ID\tCHANGED\tADDED\tREMOVED\tMANAGERS")
	for _, e := range entries {
		if e.Incomplete {
			fmt.Fprintf(tw, "%s\t-\t-\t-\t(incomplete)\n", e.ID)
			continue
		}
		managers := "-"
		if e.Managers != nil {
...
func doUpShow(stdout io.Writer, snapsDir, id string, asJSON bool) error {
	...
	if id != "" {
		...
		if target == "" {
			return fmt.Errorf("unknown up run %q", id)
		}
	}
