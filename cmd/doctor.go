...
var errDoctorFindings = i18n.Errorf("hukou doctor found state problems")
...
func doDoctor(stdout io.Writer, root string, jsonOutput, deep bool) error {
	...
	if err != nil {
		return i18n.Wrapf("render doctor report: %w", err)
	}
	if !report.Healthy() {
		return i18n.Wrapf("%w: status=%s", errDoctorFindings, report.Status)
	}
	return nil
}
