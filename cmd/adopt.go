...
adopted tool, record it with --exe so future upgrades can find it.`,
...
func init() {
	...
	adoptCmd.Flags().StringVar(&adoptExe, "exe", "", "executable name inside the release archive when it differs from the adopted name")
...
func runAdopt(cmd *cobra.Command, args []string) error {
	...
	if adoptJSON && !adoptDryRun {
		return fail(i18n.Errorf("--json requires --dry-run"))
	}
	if adoptDryRun {
		return doAdoptDryRun(cmd.OutOrStdout(), target, repoArg, adoptLocal, adoptTag, adoptForce, adoptExe, adoptJSON, runSecurityGate)
	}
	return doAdopt(cmd.OutOrStdout(), cmd.ErrOrStderr(), target, repoArg, adoptLocal, adoptTag, adoptForce, adoptExe)
}

func doAdopt(stdout, stderr io.Writer, target, repoArg string, local bool, tag string, force bool, exe string) error {
	return doAdoptWithDeps(stdout, stderr, target, repoArg, local, tag, force, exe, runSecurityGate, saveManifest)
}
...
func doAdoptDryRun(stdout io.Writer, target, repoArg string, local bool, tag string, force bool, exe string, jsonOutput bool, securityGate func(string) (*provenance.Attribution, error)) error {
	if err := ensureDryRunTransactionClean(); err != nil {
		return fail(err)
	}
	inspection, err := inspectAdoption(target, repoArg, local, tag, force, exe, securityGate)
	if err != nil {
		return fail(err)
	}
	if jsonOutput {
...
func doAdoptWithDeps(stdout, stderr io.Writer, target, repoArg string, local bool, tag string, force bool, exe string, securityGate func(string) (*provenance.Attribution, error), save func(*manifest.Manifest) error) error {
	...
	if err != nil {
		return fail(i18n.Wrapf("prepare adoption activation: %w", err))
	}
	...
	if _, err := s.PreflightOriginalPath(name, filepath.Base(binPath)); err != nil {
		return fail(i18n.Wrapf("preflight original backup: %w", err))
	}
	origPath, err := s.PrepareOriginalPath(name, filepath.Base(binPath))
	if err != nil {
		return fail(i18n.Wrapf("prepare original path: %w", err))
	}
	if _, err := os.Lstat(origPath); err == nil {
		return fail(i18n.Errorf("original backup already exists: %s", origPath))
	} else if !errors.Is(err, os.ErrNotExist) {
		return fail(err)
	}
	originalSource := binPath
	if liveTopology, err := os.Lstat(binPath); err != nil {
		return fail(i18n.Wrapf("inspect live path before adoption: %w", err))
	} else if liveTopology.Mode()&os.ModeSymlink != 0 {
		originalSource, err = filepath.EvalSymlinks(binPath)
		if err != nil {
			return fail(i18n.Wrapf("resolve live symlink before adoption: %w", err))
		}
	}
	...
	if err := afterManifest.Normalize(); err != nil {
		return fail(i18n.Wrapf("normalize adoption manifest: %w", err))
	}
	manifestBytes, err := encodeManifest(afterManifest)
	if err != nil {
		return fail(i18n.Wrapf("encode adoption manifest: %w", err))
	}
	...
	if err != nil {
		return fail(i18n.Wrapf("prepare adoption transaction: %w", err))
	}
	beforeOriginal, _ := tx.Before("original")
	if beforeOriginal.Kind != statejournal.KindAbsent {
		return fail(abortStateTransaction(tx, i18n.Errorf("original backup appeared concurrently: %s", origPath)))
	}
	...
	if err := validateTransactionStateSHA(tx, "live-guard", sha, false); err != nil {
		return fail(abortStateTransaction(tx, i18n.Wrapf("binary changed while preparing adoption: %w", err)))
	}
	if err := tx.Apply("original"); err != nil {
		return fail(abortStateTransaction(tx, i18n.Wrapf("backup original: %w", err)))
	}
	stressPause("post-prepared")
	...
	if entriesErr != nil || len(originalEntries) != 1 || originalEntries[0].Name() != filepath.Base(origPath) || !originalEntries[0].Type().IsRegular() {
		return fail(abortStateTransaction(tx, i18n.Errorf("original backup namespace changed during adoption; refusing inconsistent adoption: %v", entriesErr)))
	}
	...
	if backupErr != nil || liveErr != nil || backupStatErr != nil || liveStatErr != nil || backupSHA != sha || liveSHA != sha || modeChanged {
		return fail(abortStateTransaction(tx, i18n.Errorf("binary changed while creating original backup; refusing inconsistent adoption (backup_err=%v live_err=%v backup_stat_err=%v live_stat_err=%v)", backupErr, liveErr, backupStatErr, liveStatErr)))
	}

	if err := tx.Verify("manifest", false); err != nil {
		return fail(abortStateTransaction(tx, i18n.Wrapf("manifest changed during adoption; refusing overwrite: %w", err)))
	}
	m.Put(entry)
	...
	if err := commitStateTransaction(tx, stderr); err != nil {
		return fail(i18n.Wrapf("commit adoption transaction: %w", err))
	}
	finalizeStateTransaction(tx, stderr, name, "adoption")

	fmt.Fprintf(stdout, "%s\n", i18n.T("Adopted %s (%s) at %s", name, tag, binPath))
	if repo != "" {
		fmt.Fprintf(stdout, "%s\n", i18n.T("repo: %s", repo))
	}
	return nil
...
func inspectAdoption(target, repoArg string, local bool, tag string, force bool, exe string, securityGate func(string) (*provenance.Attribution, error)) (*adoptionInspection, error) {
	binPath, err := resolveAdoptTarget(target)
	if err != nil {
		return nil, i18n.Wrapf("locate target: %w", err)
	}
	info, err := os.Stat(binPath)
	if err != nil {
		return nil, i18n.Wrapf("stat %s: %w", err, binPath)
	}
	if !info.Mode().IsRegular() {
		return nil, i18n.Errorf("%s is not a regular file", binPath)
	}
	if info.Mode()&0o111 == 0 {
		return nil, i18n.Errorf("%s is not executable", binPath)
	}
	if info.Mode()&(os.ModeSetuid|os.ModeSetgid|os.ModeSticky) != 0 {
		return nil, i18n.Errorf("%s uses privileged/special mode bits that hukou does not preserve", binPath)
	}

	name := filepath.Base(binPath)
	if err := store.ValidateName(name); err != nil {
		return nil, i18n.Wrapf("invalid tool name: %w", err)
	}
	sha, err := store.SHA256File(binPath)
	if err != nil {
		return nil, i18n.Wrapf("sha256 %s: %w", err, binPath)
	}
	...
	if local {
		...
		if repo == "" {
			return nil, i18n.Errorf("cannot infer a repository; provide owner/repo or use --local (try: hukou suggest <%s>)", name)
		}
		if tag == "" {
			tag = "adopted"
		}
	...
	if err := manifest.ValidateRepository(repo); err != nil {
		return nil, i18n.Wrapf("invalid repository: %w", err)
	}
	if exe != "" && local {
		return nil, i18n.Errorf("--exe requires a release repository; not allowed with --local")
	}
	if exe != "" {
		if err := manifest.ValidateArchiveExe(exe); err != nil {
			return nil, i18n.Wrapf("invalid --exe: %w", err)
		}
	}
	if err := store.ValidateTag(tag); err != nil {
		return nil, i18n.Wrapf("invalid adoption tag: %w", err)
	}
	...
	if attr == nil {
		return nil, i18n.Errorf("the ownership safety check returned no result")
	}
	if !allowedAdoptSource(attr.Source) && !force {
		return nil, i18n.Errorf("%s is owned by %s (%s); use --force to override explicitly", name, attr.Source, attr.Evidence)
	}
	...
	if existing := m.Get(name); existing != nil {
		return nil, i18n.Errorf("%s is already adopted at %s; refusing to replace the entry", name, existing.Path)
	}
	cleanPath := filepath.Clean(binPath)
	for _, existing := range m.Entries {
		if filepath.Clean(existing.Path) == cleanPath {
			return nil, i18n.Errorf("path %s is already registered as %s; refusing duplicate adoption", binPath, existing.Name)
		}
	}
	...
	if err != nil {
		return nil, i18n.Wrapf("preflight original backup: %w", err)
	}
